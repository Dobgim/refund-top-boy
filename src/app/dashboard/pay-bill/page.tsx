...
import { PayBillForm } from "@/components/dashboard/banking-forms";
...
export default async function Page() {
  const [profile, account, records] = await Promise.all([
    getCurrentProfile(),
    getMyAccount(),
    getMyBankingRecords<Record<string, unknown>>("bill_payments"),
  ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Pay Bill"
        description="Settle a bill straight from your balance."
        action={
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        <Card className="p-5 sm:p-6">
          <PayBillForm balance={balance} currency={currency} />
        </Card>
