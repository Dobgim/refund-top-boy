...
import { LoanForm } from "@/components/dashboard/banking-forms";
...
export default async function Page() {
  const [profile, account, records] = await Promise.all([
    getCurrentProfile(),
    getMyAccount(),
    getMyBankingRecords<Record<string, unknown>>("loans"),
  ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Loan"
        description="Apply for credit. A reviewer decides every application."
        action={
