...
import { FixedDepositForm } from "@/components/dashboard/banking-forms";
...
export default async function Page() {
  const [profile, account, records] = await Promise.all([
    getCurrentProfile(),
    getMyAccount(),
    getMyBankingRecords<Record<string, unknown>>("fixed_deposits"),
  ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Fixed deposit (FDR)"
        description="Lock an amount for a fixed term at a fixed rate."
        action={
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        <Card className="p-5 sm:p-6">
          <FixedDepositForm balance={balance} currency={currency} />
        </Card>
