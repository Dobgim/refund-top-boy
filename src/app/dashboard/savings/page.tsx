...
import { SavingsForm } from "@/components/dashboard/banking-forms";
...
export default async function Page() {
  const [profile, account, records] = await Promise.all([
    getCurrentProfile(),
    getMyAccount(),
    getMyBankingRecords<Record<string, unknown>>("savings_schemes"),
  ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="DPS savings"
        description="A recurring savings scheme with a fixed monthly deposit."
        action={
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        <Card className="p-5 sm:p-6">
          <SavingsForm balance={balance} currency={currency} />
        </Card>
